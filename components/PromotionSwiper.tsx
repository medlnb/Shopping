          ...
          <div className="flex flex-col-reverse md:items-center md:flex-row gap-2">
            <div className="flex-1 h-full flex flex-col items-start justify-between pb-2 gap-4">
              <p className="text-gray-700">
                <b className="text-4xl font-bold text-blue-600">
                  {product.dicount}%
                </b>
                {" Off"}
              </p>
              <div>
                <p className="text-xl font-semibold"> {product.tilte}</p>
                <p className="text-sm text-gray-600">{product.description}</p>
              </div>
              <Link
                href={`/product/${product._id}`}
                className="bg-[#1c274c] hover:bg-[#33447c] duration-150 px-4 py-2 rounded-md text-white text-sm"
              >
                Shop Now
              </Link>
            </div>
